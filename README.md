...
<head>
    ...
    <title>Barber Pro - Gestão de Caixa com Edição</title>
    <style>
...
<div class="container">
    ...
    <div class="card">
        ...
        <button class="btn-add" onclick="registrar(true)">Lançar Entrada</button>
        <button class="btn-out" onclick="registrar(false)">Lançar Saída (Gasto)</button>
    </div>

    <div class="grid-resumo">
        <div class="resumo-item"><span>Total Bruto</span><strong id="total-bruto">R$ 0,00</strong></div>
        <div class="resumo-item"><span>Comissões</span><strong id="total-comissao">R$ 0,00</strong></div>
        <div class="resumo-item"><span>Gastos</span><strong id="total-saidas" style="color:var(--red)">R$ 0,00</strong></div>
        <div class="resumo-item"><span>Líquido</span><strong id="total-liquido" style="color:var(--green)">R$ 0,00</strong></div>
    </div>
    ...
    <div class="card">
        <h3>Histórico de Dias Anteriores</h3>
        <div id="log-dias"></div>
...
<div id="modal-edicao">
    <div class="modal-content">
        ...
        <button class="btn-add" onclick="salvarEdicao()">Gravar Alterações</button>
...
<script>
    ...
    function registrar(isEntrada) {
        ...
        if (isEntrada) {
            ...
            let valorComissao = (barbeiro !== "Proprietário") ? (valor * 0.5) : 0;
            ...
            caixaAtual.vendas.push({ desc: `GASTO/SAÍDA`, valor, hora, tipo: 'saida' });
        }
    ...
    function fecharDia() {
        if (caixaAtual.vendas.length === 0) return alert("Não há vendas para fechar o dia.");
        
        if (confirm("Deseja encerrar o caixa de hoje? Os dados serão movidos para o histórico.")) {
        ...
        historicoDias.forEach((dia, index) => {
            container.innerHTML += `
                <div class="registro-dia">
                    <strong>📅 Dia: ${dia.data}</strong><br>
                    Total Bruto: R$ ${dia.bruto.toFixed(2)} | Comissão: R$ ${dia.comissao.toFixed(2)}<br>
                    Saídas: R$ ${dia.saidas.toFixed(2)} | <span style="color:var(--green)">Líquido: R$ ${dia.liquido.toFixed(2)}</span><br>
    ...
    function salvarEdicao() {
        ...
        if (isNaN(bruto) || isNaN(comissao) || isNaN(saidas)) return alert("Valores inválidos");
    ...
    function apagarTudo() {
        if(confirm("Isso apagará TODO o seu histórico de dias salvos. Tem certeza?")) {
            localStorage.clear();
            location.reload();
        }
    }
